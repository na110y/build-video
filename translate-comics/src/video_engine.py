"""Text → Speech → MP4 video engine.
Uses edge-tts (free, no API key) + ffmpeg (needs to be installed separately).

Output: a black-background MP4 with white subtitles and Vietnamese AI voice.
"""
import re
import json
import subprocess
...
from src.tts_engine import tts_sync
...
def _split_into_subtitle_chunks(text: str, max_chars: int = 60) -> List[str]:
    ...
    for part in raw:
        ...
        if len(part) <= max_chars:
            ...
            # Split long sentence by comma / phrase
...
def _generate_srt(chunks: List[str], output_path: Path) -> None:
    ...
    for chunk in chunks:
        ...
        lines.append("")
        idx += 1
        current_ms = end_ms + 300  # 300ms gap between subtitles
...
def text_to_video(
    ...
    print("  [TTS] Dang tao giong noi...")
    tts_sync(text, mp3_path, voice=voice, rate=rate)
    ...
    # Use drawtext instead of subtitles filter for better font control on Windows
    # First convert SRT to ass for styling, or use drawtext directly
    # Simpler: use subtitles filter with styling
    cmd = [
        "ffmpeg", "-y",
        ...
        "-vf", (
            ...
            f"'FontSize={font_size},PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,"
            f"BackColour=&H00000000,Outline=2,Shadow=0,Alignment=2,MarginV=50'"
        ),
...
def video_from_file(
    ...
    **kwargs,
    ...
    return text_to_video(text, output_path, **kwargs)
